...
> [!NOTE]
> Windows 操作系统需要安装`Microsoft Edge WebView2`运行时。

### Microsoft Windows 系统 [前端完备]

`Kelp`经过了`Windows 10/11`操作系统测试，前端UI效果完备，使用体验好。

#### Windows 11

`Windows 11`操作系统已经内置了`Microsoft Edge WebView2`运行时，无须另行安装。

通过[Releases](https://github.com/bayuncao/kelp/releases)下载对应的`kelp_windows_*.exe`版本即可运行Kelp。

#### Windows 10

某些版本的`Windows 10`操作系统需要安装`Microsoft Edge WebView2`运行时，推荐通过此[链接](https://developer.microsoft.com/zh-cn/microsoft-edge/webview2/?form=MA13LH#download)跳转至官方下载地址，安装**常青独立安装程序版本**。

如果你的PC在打开Kelp程序时提示了错误，那么请务必安装`Microsoft Edge WebView2`运行时。

后续步骤通过[Releases](https://github.com/bayuncao/kelp/releases)下载对应的`kelp_windows_*.exe`版本即可运行Kelp。
